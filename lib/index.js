...
var aconsole = new Proxy(
    {
        ...
        $show: function () {
            ...
            self._data.forEach(function (item) {
                ...
                if (item.type === 'input') {
                    console.info('%c📝INPUT:','color:#98c379;')
                }
                if (item.type === 'output') {
                    console.info('%c📇OUTPUT:','color:#61aeee;')
                }
        ...
        $input: function (...arg) {
            ...
            self._data.push({
                ...
                type: 'input',
                time: new Date().getTime(),
                arguments: [`\r\n${functionName}(`].concat(arg.map(clone).map(stringFormat).map(appendComma)).concat(')')
            })
        },
        $output: function (functionName, data) {
            ...
            self._data.push({
                ...
                type: 'output',
                time: new Date().getTime(),
                arguments: [`\r\n${functionName}(...) // return`].concat(
                    [data].map(clone).map(stringFormat)[0]
                )
            })
        }
    }
